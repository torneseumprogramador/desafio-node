...
export class ClienteRepository {
  ...
  public async paginate(page: number = 1, limit: number = 10): Promise<IPaginatedData<Cliente>> {
    const offset = (page - 1) * limit
    
    const [countResult] = await Database.from(this.table).count('* as total')
    const total = Number(countResult.total)
    
    const clientes = await Database.from(this.table)
      .select('*')
      .orderBy('nome', 'asc')
      .offset(offset)
      .limit(limit)
