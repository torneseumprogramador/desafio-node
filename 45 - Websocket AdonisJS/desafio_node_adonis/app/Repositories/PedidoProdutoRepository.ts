...
export class PedidoProdutoRepository {
  ...
  public async create(data: Omit<IPedidoProduto, 'id'>, trx?: any): Promise<void> {
    const { produto, ...dataToInsert } = data
    const query = trx ? trx.table(this.table) : Database.table(this.table)
    await query.insert(dataToInsert)
  }
