...
export class PedidoRepository {
  ...
  public async deleteOldOrders(year: number): Promise<void> {
    const pedidos = await Database.from(this.table)
      .where('data_entrega', '<', `${year}-01-01`)
      .select('id')

    const pedidoProdutoRepo = new PedidoProdutoRepository()

    for (const pedido of pedidos) {
      await pedidoProdutoRepo.deleteByPedidoId(pedido.id)
    }

    const result = await Database.from(this.table)
      .where('data_entrega', '<', `${year}-01-01`)
      .delete()
    
    console.log(`Quantidade de pedidos deletados: ${result}`)
  }

  public async findById(id: number, trx?: any): Promise<Pedido | null> {
    const query = trx ? trx.from(this.table) : Database.from(this.table)
  ...
  public async create(data: Omit<IPedido, 'id' | 'created_at' | 'updated_at'>): Promise<Pedido> {
    ...
    try {
      ...
      const pedido = await this.findById(id, trx)
  ...
  public async update(id: number, data: Partial<Omit<IPedido, 'id' | 'created_at' | 'updated_at'>>): Promise<Pedido> {
    ...
    try {
      ...
      const pedidoProdutoRepo = new PedidoProdutoRepository()
      await pedidoProdutoRepo.deleteByPedidoId(id)
